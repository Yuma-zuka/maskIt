...
class HomeWindow:
    def __init__(self):
        ...
        # 顔データの登録を実行するボタン
        registration_process_btn = tk.Button(self.root, text="register", command=self.do_registration_process, bg="#1995ad", fg="#537072", font=FONT) # do_registration_processメソッドの呼び出し
        registration_process_btn.place(relx=0.5, rely=0.2, anchor=tk.CENTER)
        # 顔データファイルの管理を実行するボタン
        manage_process_btn = tk.Button(self.root, text="manage", command=self.do_management, bg="#a1d6e2", fg="#537072", font=FONT) # do_managementメソッドの呼び出し
        manage_process_btn.place(relx=0.5, rely=0.4, anchor=tk.CENTER)
        # 写真の処理を実行するボタン
        image_process_btn = tk.Button(self.root, text="image", command=self.do_imagerecpro, bg="#a1d6e2", fg="#537072",  font=FONT) # do_imagerecproメソッドの呼び出し
        image_process_btn.place(relx=0.5, rely=0.6, anchor=tk.CENTER)
        # リアルタイムで顔の処理を実行するボタン
        real_time_process_btn = tk.Button(self.root, text="realTime", command=self.do_real_time_rec, bg="#a1d6e2", fg="#537072", font=FONT) # do_real_time_recメソッドの呼び出し
