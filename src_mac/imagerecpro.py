...
class ImageRecPro:
    ...
    def make_result_window(self, error):
        ...
        back_button = Button(self.root, text="戻る", bg="#bcbabe", fg="#000000", command=self.back_home, font=("Helvetica", 50), relief="raised") # ホームに戻るボタンの設定 back_homeメソッドの呼び出し
        back_button.place(x=30,y=740) # "戻る"ボタンの配置
        retry = Button(self.root, text="続けて編集する", bg="#bbdceb", fg="#536f72", command=self.re_rec_image, font=("Helvetica", 50), relief="raised") # 続けて編集するボタンの設定 re_rec_imageメソッドの呼び出し
