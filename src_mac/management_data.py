...
class ManageData:
    ...
    def manage(self):
        ...
        back_button = Button(self.root, text="戻る", command=self.back_home, font=("Helvetica", 50)) # ホームに戻るボタンの設定 back_homeメソッドの呼び出し
        back_button.place(x=30,y=740) # "戻る"ボタンの配置
        enter = Button(self.root, text="実行", command=self.enter_changing, font=("Helvetica", 50)) # 実行するボタンの設定 enter_changingメソッドの呼び出し
        enter.place(x=1200,y=740) # "実行"ボタンの配置
        remove_button = Button(self.root, text="ファイルを削除", command=self.remove_file, font=("Helvetica", 80)) # ファイル削除ボタンの設定 remove_fileメソッドの呼び出し
        remove_button.place(x=80,y=50) # "ファイルを削除"ボタンの配置
        rename_button = Button(self.root, text="ファイル名変更", command=self.rename_file, font=("Helvetica", 80)) # ファイル名変更ボタンの設定 rename_fileメソッドの呼び出し
