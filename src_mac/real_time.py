...
class RealTimeRec(tk.Frame):
    def __init__(self, master = None):
        ...
        back_button = Button(self.master, text="戻る", command=self.back_home, font=("Helvetica", 50)) # ホームに戻るボタンの設定 back_homeメソッドの呼び出し
        back_button.place(x=80,y=740, anchor="center") # "戻る"ボタンの配置
        filter_change_button = Button(self.master, text="フィルター変更", command=self.change_filter, font=("Helvetica", 50)) # フィルターを変更するボタンの設定 change_filterメソッドの呼び出し
        ...
        photo_button = Button(self.master, text="写真を撮る", command=self.take_picture, font=("Helvetica", 50)) # 写真を撮るボタンの設定 take_pictureメソッドの呼び出し
